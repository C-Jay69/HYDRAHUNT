...
import { Zap, Layout, CreditCard, LogOut, User as UserIcon } from 'lucide-react';
    ...
    <nav className="sticky top-0 z-50 bg-black border-b border-[#333] px-4 py-3 flex items-center justify-between">
      <div className="flex items-center gap-3">
        <div className="w-10 h-10 overflow-hidden border border-[#00FFFF] shadow-[0_0_10px_rgba(0,0,255,0.5)]">
            <img src="/hydra-logo.png" alt="Hydra Logo" className="w-full h-full object-cover" />
        </div>
      ...
      <div className="hidden md:flex gap-6">
        <NavLink to="/dashboard" icon={<Layout size={18} />} label="Dashboard" active={isActive('/dashboard')} />
        <NavLink to="/templates" icon={<Zap size={18} />} label="Templates" active={isActive('/templates')} />
        <NavLink to="/pricing" icon={<CreditCard size={18} />} label="Pro" active={isActive('/pricing')} />
      </div>
         ...
         {user ? (
           <>
             <div className="hidden md:flex flex-col items-end leading-tight">
                <span className="text-xs font-bold text-[#00FFFF]">OPERATOR</span>
             ...
             <button onClick={handleLogout} className="p-2 border border-[#333] bg-[#111] text-gray-400 hover:text-white hover:border-[#FF00FF] transition-colors" title="Logout">
                <LogOut size={18} />
             </button>
           </>
         ) : (
